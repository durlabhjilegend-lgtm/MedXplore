...
def compute_sla_status(tasks: list[dict], sla: dict) -> dict:
    ...
    # Available capacity
    team_capacity_hours = (
        days_remaining * sla.get("team_hours_per_day", 8) * sla.get("team_size", 5)
    )

    shortfall_hours = total_hours_remaining - team_capacity_hours
    will_miss = shortfall_hours > 0

    # Which tasks are critical and at risk?
    critical_remaining = [
        t for t in tasks
...
def identify_reassignment_options(tasks: list[dict], sla_status: dict) -> dict:
    """Find tasks that can be deprioritized and resources freed for critical work."""
    # Low/medium tasks that are not started
    deferrable = [
        t for t in tasks
        if t["priority"] in ("LOW", "MEDIUM") and t["status"] == "NOT_STARTED"
    ]

    # Workers currently on low-priority tasks
    ...
    # Critical tasks not yet started or in progress with low completion
    at_risk_critical = [
        t for t in tasks
        if t["priority"] in ("CRITICAL", "HIGH") and t["status"] != "DONE"
        and (t["completed_hours"] / t["estimated_hours"] < 0.5 if t["estimated_hours"] > 0 else True)
    ]

    hours_reclaimable = sum(t["estimated_hours"] - t["completed_hours"]
                            for t in deferrable)
...
OUTPUT FORMAT — respond ONLY with valid JSON, no markdown, no preamble:
{
  ...
  "recovery_plan": [
    {
      "action_id": 1,
      "action_type": "REASSIGN | DEFER | ESCALATE | OVERTIME",
...
def run_ai_recovery_plan(tasks, sla_status, options, api_key: str) -> dict:
    ...
    response = client.messages.create(
        ...
        messages=[{"role": "user", "content":
            f"Analyze this SLA breach risk and produce a specific recovery plan:\n{json.dumps(payload, indent=2, default=str)}"}]
    )
...
def print_report(tasks, sla_status, options, ai_result=None):
    print("\n" + "═" * 65)
    print("  🛡️  SLA PENALTY PREVENTION REPORT")
    print("═" * 65)
    status_icon = "🚨" if sla_status["will_miss_sla"] else "✅"
    print(f"  {status_icon} SLA: {sla_status['sla_name']}")
    ...
    print(f"  Penalty at risk   : ₹{sla_status['projected_penalty_inr']:,.0f}  [MAX: ₹{sla_status['max_penalty_inr']:,.0f}]")
    print(f"  Risk level        : {sla_status['risk_level']}")
    print("─" * 65)
    print(f"  Reclaimable hours from deferral: {options['hours_reclaimable']:.0f}h  |  Feasible: {'YES' if options['reassignment_feasible'] else 'NO'}")
    print("─" * 65)

    if ai_result:
        icon = "✅" if ai_result.get("recovery_feasible") else "❌"
        print(f"\n  {icon} Recovery feasible: {ai_result.get('recovery_confidence', '?')} confidence")
        print(f"  Penalty preventable: ₹{ai_result.get('penalty_preventable_inr', 0):,.0f}")
        print(f"\n  RECOVERY PLAN ({len(ai_result.get('recovery_plan', []))} actions):\n")
        for a in ai_result.get("recovery_plan", []):
            mode_icon = {"AUTO_EXECUTE": "✅", "STAGE_FOR_APPROVAL": "⏳", "ESCALATE": "🚨"}.get(a.get("execution_mode"), "📋")
            print(f"  [{a['action_id']}] {mode_icon} {a['action_type']:<12} {a['description'][:55]}")
            print(f"       Approver: {a.get('approval_level','?')} | Hours reclaimed: {a.get('hours_reclaimed',0):.0f}h | Impact: ₹{a.get('financial_impact_inr',0):,.0f}")
        ...
        for t in options["at_risk_critical_tasks"][:5]:
            ...
            print(f"  ⚠️  {t['task_id']}: {t['task_name']} ({rem:.0f}h remaining) — {t['assigned_to']}")
...
def main():
    ...
    if sla_status["will_miss_sla"]:
        print(f"🚨 SLA BREACH PROJECTED — ₹{sla_status['projected_penalty_inr']:,.0f} at risk")
    else:
        print("✅ SLA on track — monitoring only")
